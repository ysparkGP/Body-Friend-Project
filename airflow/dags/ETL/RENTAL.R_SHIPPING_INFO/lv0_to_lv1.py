...
@dag(
    dag_id = "lv0_dag_RENTAL.R_SHIPPING_INFO",
...
def lv0_job():
    ...
    def lv0_job_func(**context):
        ...
        with postgres_conn.cursor() as postgres_cursor:
            sql = f"select lv1.func_daily_r_shipping_info();"
            result = postgres_hook.get_records(sql)

            if not result[0][0]: raise AirflowException("lv1.func_daily_r_shipping_info: Failed.")
