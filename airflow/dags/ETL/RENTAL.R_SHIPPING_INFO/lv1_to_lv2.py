...
@dag(
    dag_id = "lv1_dag_RENTAL.R_SHIPPING_INFO",
...
def lv1_job():
    ...
    def lv1_job_func(**context):
        ...
        with postgres_conn.cursor() as postgres_cursor:
            sql = f"select lv2.func_lv2_to_hc_shipping_info();"
            result = postgres_hook.get_records(sql)

            if not result[0][0]: raise AirflowException("lv2.func_lv2_to_hc_shipping_info: Failed.")
