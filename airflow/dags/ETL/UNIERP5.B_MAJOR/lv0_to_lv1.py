...
@dag(
    dag_id = "lv0_dag_UNIERP5.B_MAJOR",
...
def lv0_job():
    ...
    def check_condition(**context):
        ...
        else: raise AirflowException('legacy_to_lv0_UNIERP5.B_MINOR 가 아직 완료되지 않은 것 같음... 30분 뒤 재시도')
    ...
    def lv0_job_func(**context):
        ...
        with postgres_conn.cursor() as postgres_cursor:
            sql = f"select lv1.func_daily_code_master();"
            result = postgres_hook.get_records(sql)

            if not result[0][0]: raise AirflowException("lv1.func_daily_code_master: Failed.")
