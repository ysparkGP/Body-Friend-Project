...
@dag(
    dag_id = "lv0_dag_RENTAL.R_PROD_SYMPTOM_SML",
...
def lv0_job():
    ...
    def check_condition(**context):
        ...
        else: raise AirflowException('R_PROD_SYMPTOM_SML 테이블이 아직 완성되지 않음')
    ...
    def lv0_job_func(**context):
        ...
        with postgres_conn.cursor() as postgres_cursor:
            sql = f"select lv1.func_daily_r_svc_type_sml();"
            result = postgres_hook.get_records(sql)

            if not result[0][0]: raise AirflowException("lv1.func_daily_r_svc_type_sml: Failed.")
