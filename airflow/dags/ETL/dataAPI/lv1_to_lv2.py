...
@dag(
    dag_id = "lv1_dag_DATAAPI.PUBLIC_INSTITUTION",
...
def lv1_job():
    ...
    def lv1_job_func(**context):
        ...
        with postgres_conn.cursor() as postgres_cursor:
            sql = f"select lv1.func_institution_addr_seq_mark();"
            result = postgres_hook.get_records(sql)

            if not result[0][0]: raise AirflowException("lv1.func_institution_addr_seq_mark: Failed.")
            else: 
                sql = f"select lv2.func_lv2_to_hc_public_institution();"
                result = postgres_hook.get_records(sql)
                if not result[0][0]: raise AirflowException("lv2.func_lv2_to_hc_public_institution: Failed.")
