...
@dag(
    dag_id = "lv0_dag_DATAAPI.PUBLIC_INSTITUTION",
...
def lv0_job():
    ...
    def check_condition(**context):
        ...
        else: raise AirflowException('6개 공공기관 lv0 작업 확인... 30분 후 재시도')
    ...
    def lv0_job_func(**context):
        ...
        with postgres_conn.cursor() as postgres_cursor:
            sql = f"select lv1.func_public_institution();"
            result = postgres_hook.get_records(sql)

            if not result[0][0]: raise AirflowException("lv1.func_public_institution: Failed.")
