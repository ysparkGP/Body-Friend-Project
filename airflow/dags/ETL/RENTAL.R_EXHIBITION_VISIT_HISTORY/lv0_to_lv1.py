...
@dag(
    dag_id = "lv0_dag_RENTAL.R_EXHIBITION_VISIT_HISTORY",
...
def lv0_job():
    ...
    def lv0_job_func(**context):
        ...
        with postgres_conn.cursor() as postgres_cursor:
            sql = f"select lv1.func_daily_r_exhibition_visit();"
            result = postgres_hook.get_records(sql)

            if not result[0][0]: raise AirflowException("lv1.func_daily_r_exhibition_visit: Failed.")
