...
@dag(
    dag_id = "lv1_dag_RENTAL.R_SHIPPING_EVALUATION",
...
def lv1_job():
    ...
    def check_condition(**context):
        ...
        else: raise AirflowException('lv2.func_lv2_to_hc_shipping_info() 가 완료되지 않은 거 같음... 30분 뒤 재실행')
    ...
    def lv1_job_func(**context):
        ...
        with postgres_conn.cursor() as postgres_cursor:
            sql = f"select lv2.func_lv2_to_hc_shipping_evaluation();"
            result = postgres_hook.get_records(sql)

            if not result[0][0]: raise AirflowException("lv2.func_lv2_to_hc_shipping_evaluation: Failed.")
