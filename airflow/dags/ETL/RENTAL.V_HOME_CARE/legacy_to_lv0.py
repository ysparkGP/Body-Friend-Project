...
@dag(
    dag_id = "legacy_to_lv0_RENTAL.V_HOME_CARE",
...
def etl_dag():
    ...
    def etl_mssql(**context):
        ...
        while start_page <= total_cnt:
            sql = f"SELECT A.* FROM( SELECT *, ROW_NUMBER() OVER(ORDER BY CONT_DT) AS row_num FROM {schema_name}.DBO.{table_name} ) AS A WHERE 1=1 AND A.row_num BETWEEN {start_page} AND {start_page+per_page}-1 ";
